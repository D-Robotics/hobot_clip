...
GetTextFeatureClient::GetTextFeatureClient(const rclcpp::NodeOptions & options)
  : Node("text_action_client", options)
{
...
int GetTextFeatureClient::send_goal(std::vector<std::string>& texts, 
  ...
  goal_msg.type = false;
  goal_msg.texts = std::move(texts);

  RCLCPP_WARN_STREAM(this->get_logger(), "Sending goal, type: "
    << goal_msg.type << ", texts: " << goal_msg.texts[0]);

  auto send_goal_options = rclcpp_action::Client<GetFeatures>::SendGoalOptions();
  send_goal_options.goal_response_callback =
    std::bind(&GetTextFeatureClient::goal_response_callback, this, _1);
  send_goal_options.feedback_callback =
    std::bind(&GetTextFeatureClient::feedback_callback, this, _1, _2);
  send_goal_options.result_callback =
    std::bind(&GetTextFeatureClient::result_callback, this, _1);
...
void GetTextFeatureClient::goal_response_callback(GoalHandleGetFeatures::SharedPtr goal_handle)
{
  if (!goal_handle) {
...
void GetTextFeatureClient::feedback_callback(
...
void GetTextFeatureClient::result_callback(const GoalHandleGetFeatures::WrappedResult & result)
{
  bool succseed = false;
  std::stringstream ss;
